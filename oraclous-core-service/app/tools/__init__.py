...
from app.tools.registry import tool_registry  # Fixed: was ToolRegistry
...
def _register_all_tools():
    ...
    try:
        from app.tools.implementations.ingestion.google_drive_reader import (
            GoogleDriveReader,
        )

        tool_registry.register_tool(
            GoogleDriveReader, GoogleDriveReader.get_tool_definition()
        )
    ...
    try:
        from app.tools.implementations.ingestion.postgresql_reader import (
            PostgreSQLReader,
        )

        tool_registry.register_tool(
            PostgreSQLReader, PostgreSQLReader.get_tool_definition()
        )
    except ImportError as e:
...
# FIXED: Auto-register tools on module import
_register_all_tools()

# FIXED: Export main classes

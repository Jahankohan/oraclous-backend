...
class ToolSyncService:
    ...
    async def _sync_memory_to_db(
        self, memory_tools: List[ToolDefinition], db_registry: ToolRegistryService
    ):
        """
        ...
        for tool_definition in memory_tools:
            try:
                ...
                logger.info(
                    f"Creating tool in DB: {tool_definition.name} ({tool_definition.id})"
                )
                ...
                if success:
                    ...
                    logger.error(
                        f"❌ Failed to create tool in DB: {tool_definition.name}"
                    )
                ...
                logger.error(
                    f"Error creating tool {tool_definition.name} in DB: {str(e)}"
                )
                self.failed_tools.append(
                    {
                        "id": tool_definition.id,
    ...
    async def _sync_db_to_memory(self, tool_definition: ToolDefinition):
        ...
        try:
            ...
            logger.debug(
                f"Successfully synced tool: {tool_definition.name} ({tool_id})"
            )
    ...
    async def _handle_existing_tool(
        self, db_definition: ToolDefinition, memory_definition: ToolDefinition
    ):
        """
        ...
        try:
            ...
            if db_updated > memory_updated:
                ...
                logger.debug(
                    f"Updated tool {db_definition.id} in memory registry (DB version newer)"
                )
            else:
                logger.debug(
                    f"Kept existing tool {db_definition.id} in memory registry (memory version newer or same)"
                )

        except Exception as e:
            logger.warning(
                f"Could not compare timestamps for tool {db_definition.id}: {str(e)}. Using DB version."
            )
    ...
    async def validate_tool_availability(self, tool_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "available_for_execution": definition is not None
            and executor_class is not None,

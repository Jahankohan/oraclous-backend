...
class InstanceRepository:
    ...
    async def create_instance(
        self, user_id: UUID, request: CreateInstanceRequest
    ) -> ToolInstance:
    ...
    async def get_user_instance(
        ...
        query = select(ToolInstanceDB).where(
            and_(
                ToolInstanceDB.id == instance_id, ToolInstanceDB.user_id == str(user_id)
            )
        )
        result = await self.db.execute(query)
        db_instance = result.scalar_one_or_none()
    ...
    async def update_instance_execution_stats(
        ...
        new_execution_count = (
            current_instance.execution_count + execution_count_increment
        )
    ...
    async def update_instance(
        self, instance_id: str, user_id: UUID, request: UpdateInstanceRequest
    ) -> Optional[ToolInstance]:
        """Update instance"""
        # Build update data
        update_data = {}
    ...
    async def configure_credentials(
        self, instance_id: str, user_id: UUID, credential_mappings: Dict[str, str]
    ) -> Optional[ToolInstance]:
        ...
        query = (
            ...
            .values(
                credential_mappings=credential_mappings, updated_at=datetime.utcnow()
            )
        )
    ...
    async def get_instances_by_workflow(self, workflow_id: str) -> List[ToolInstance]:
        ...
        query = (
            select(ToolInstanceDB)
            .where(ToolInstanceDB.workflow_id == workflow_id)
    ...
    async def create_execution(
        self, user_id: UUID, request: CreateExecutionRequest
    ) -> Execution:
        ...
        if not instance:
            raise ValueError(
                f"Instance {request.instance_id} not found for user {user_id}"
            )
    ...
    async def update_execution(
        ...
        query = (
            update(ExecutionDB).where(ExecutionDB.id == execution_id).values(**updates)
        )
    ...
    async def list_executions(
        ...
        executions = [
            self._db_execution_to_pydantic(db_exec) for db_exec in db_executions
        ]
    ...
    def _db_instance_to_pydantic(self, db_instance: ToolInstanceDB) -> ToolInstance:
        ...
        return ToolInstance(
            ...
            total_credits_consumed=Decimal(
                str(db_instance.total_credits_consumed or 0)
            ),
    ...
    def _db_execution_to_pydantic(self, db_execution: ExecutionDB) -> Execution:
        ...
        return Execution(
            ...
            processing_time_ms=int(db_execution.processing_time_ms)
            if db_execution.processing_time_ms
            else None,

...
class WorkflowDB(Base, UUIDMixin, TimestampMixin):
    ...
    instances = relationship(
        "ToolInstanceDB", back_populates="workflow", cascade="all, delete-orphan"
    )
    executions = relationship(
        "WorkflowExecutionDB", back_populates="workflow", cascade="all, delete-orphan"
    )
...
class WorkflowShareDB(Base, UUIDMixin, TimestampMixin):
    ...
    shared_with_id = Column(
        UUID(as_uuid=True), nullable=True
    )  # Specific user (null for public)

    # Permissions
    permission_type = Column(
        String(20), nullable=False, default="VIEW"
    )  # VIEW, EXECUTE, EDIT, ADMIN
    ...
    def __repr__(self):
        return (
            f"<WorkflowShare {self.workflow_id} -> {self.shared_with_id or 'PUBLIC'}>"
        )

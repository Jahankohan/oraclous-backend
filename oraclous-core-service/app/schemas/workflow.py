...
class WorkflowNode(BaseModel):
    ...
    tool_definition_id: Optional[uuid.UUID] = Field(
        None, description="Tool definition ID if this is a tool node"
    )
    instance_id: Optional[uuid.UUID] = Field(
        None, description="Tool instance ID if configured"
    )
    ...
    position: Dict[str, float] = Field(
        default_factory=dict, description="Node position in workflow editor"
    )

    # Execution settings
    is_required: bool = Field(
        default=True, description="Whether this node is required for workflow success"
    )
...
class WorkflowEdge(BaseModel):
    ...
    condition: Optional[Dict[str, Any]] = Field(
        None, description="Conditional logic for this edge"
    )
    data_mapping: Optional[Dict[str, str]] = Field(
        None, description="Maps output fields to input fields"
    )

    # Edge metadata
    label: Optional[str] = Field(None, description="Edge label for UI")
    is_conditional: bool = Field(
        default=False, description="Whether this edge has conditions"
    )
...
class Workflow(BaseModel):
    ...
    chat_history: List[Dict[str, Any]] = Field(
        default_factory=list, description="Creation conversation"
    )
...
class CreateWorkflowRequest(BaseModel):
    ...
    generation_prompt: Optional[str] = Field(
        None, description="Natural language prompt for LangGraph"
    )
...
class WorkflowTemplate(BaseModel):
    ...
    required_tools: List[uuid.UUID] = Field(
        default_factory=list, description="Required tool definition IDs"
    )
...
class WorkflowShare(BaseModel):
    ...
    shared_with_id: Optional[uuid.UUID] = Field(
        None, description="User ID (null for public)"
    )
...
class GenerateWorkflowRequest(BaseModel):
    ...
    preferred_tools: List[str] = Field(
        default_factory=list, description="Preferred tool definition IDs"
    )
...
class GenerateWorkflowResponse(BaseModel):
    ...
    alternative_approaches: List[Dict[str, Any]] = Field(
        default_factory=list, description="Alternative workflow approaches"
    )

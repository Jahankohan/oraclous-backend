...
async def list_workflows(
    user_id: str, service: WorkflowService = Depends(get_workflow_service)
):
...
async def update_workflow(
    ...
    workflow = await service.repository.update_workflow(
        workflow_id, updates.dict(exclude_unset=True)
    )
    if not workflow:
        raise HTTPException(status_code=404, detail="Workflow not found")
    return workflow
...
async def generate_from_prompt(
    prompt: str, service: WorkflowService = Depends(get_workflow_service)
):

...
every graph the pipeline produces, and entity-level semantic search silently
returns nothing — ``graph_search``, ``hybrid_cypher_search``, the entity
deduplicator's embedding pass, and federation entity-resolution all depend on
that index.

This service computes a 3072-dim embedding for every ``:__Entity__`` that
lacks one and writes it to ``e.embedding``. It has two callers, one code
path:

  * the ingest pipeline calls it as a post-dedup stage so new graphs are
    born with embedded entities;
  * the one-time backfill calls it for graphs ingested before this stage
    existed.

...
from neo4j_graphrag.experimental.components.types import LexicalGraphConfig, DocumentInfo, Neo4jGraph
...
class AdvancedGraphRAGPipeline:
        ...
        try:
            ...
            for entity_type in ["Person", "Organization", "Concept"]:
                try:
                    ...
                    self.logger.debug(f"Schema-specific index for {entity_type} may not be needed: {e}")
    ...
    def _create_fulltext_indexes(self):
        ...
        try:
            ...
            self.logger.warning(f"Fulltext index creation failed (may already exist): {e}")
    ...
    def _create_advanced_pipeline(self, schema: GraphSchema) -> Pipeline:
        ...
        pipeline.connect("relationship_embedder", "entity_writer", input_config={"graph": "relationship_embedder"})
        ...
        try:
            with self.driver.session(database=self.config.neo4j_database) as session:
                ...
                if len(vector_indexes) >= 2:
                    assessment["strengths"].append("Multiple vector indexes for entities and chunks")
                if structure_stats["documents"] > 0:
                    assessment["strengths"].append("Proper Document-Chunk-Entity structure")
                ...
                if len(fulltext_indexes) == 0:
                    assessment["missing_critical"].append("Fulltext indexes for hybrid search")
                    assessment["readiness_score"] -= 0.15
                
                if structure_stats["entity_types"] < 3:
                    assessment["missing_critical"].append("Diverse entity types for rich retrieval")
    ...
    async def create_retrieval_system(self) -> Dict[str, Any]:
        ...
        try:
            ...
            text2cypher_retriever = LoggingText2CypherRetriever(text2cypher_retriever, self.logger)
    ...
    async def run_benchmarks(self) -> Dict[str, Any]:
        ...
        try:
            if self.embedder:
                ...
                start_time = time.time()
                test_embedding = self.embedder.embed_query("TechNova Corporation artificial intelligence")
                ...
                with self.driver.session(database=self.config.neo4j_database) as session:
                    ...
                    benchmark_results["vector_search_results"] = result["results"] if result else 0

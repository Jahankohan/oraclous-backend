...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "entities_extracted", sa.Integer(), nullable=False, server_default="0"
        ),
        sa.Column("error_message", sa.Text(), nullable=True),
        sa.Column("metadata", JSONB(), nullable=True),
    )
    op.create_index(
        "idx_sync_logs_connector_id", "connector_sync_logs", ["connector_id"]
    )
    ...
    op.create_unique_constraint(
        "uq_webhook_dedup", "webhook_events", ["connector_id", "payload_hash"]
    )
    op.create_index(
        "idx_webhook_events_connector_id", "webhook_events", ["connector_id"]
    )

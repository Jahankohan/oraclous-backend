...
class QueryClassifier:
    ...
    def classify_query(
        ...
        if any(keyword in query_lower for keyword in self.relationship_keywords):
            ...
            if any(
                word in query_lower
                for word in ["how", "what kind", "type of", "nature of"]
            ):
...
class GraphRAGChatbot:
    ...
    async def _initialize_text2cypher(self):
        ...
        try:
            ...
            examples = [
                ...
                "Get high confidence relationships -> MATCH (e1:__Entity__)-[r]->(e2:__Entity__) WHERE r.confidence > 0.8 RETURN e1.name, type(r), e2.name, r.confidence ORDER BY r.confidence DESC LIMIT 10",
                "Find entities by type -> MATCH (e:__Entity__) WHERE e.type CONTAINS 'Person' RETURN e.name, e.description LIMIT 10",
                "Get document context for entity -> MATCH (e:__Entity__)-[:FROM_CHUNK]->(c:Chunk)-[:FROM_DOCUMENT]->(d:Document) WHERE e.name = $entity_name RETURN d.path, c.text",
            ]
            ...
            # Wrap with logging
            self.text2cypher_retriever = LoggingText2CypherRetriever(
    ...
    async def chat(self, user_message: str, session_id: str = None) -> ChatMessage:
        ...
        try:
            ...
            query_type, retrieval_strategy = self.query_classifier.classify_query(
                user_message, context
            )

            self.logger.info(
                f"🎯 Query classified as {query_type.value} -> using {retrieval_strategy.value}"
            )

            # Retrieve relevant information
            retrieval_results = await self._retrieve_information(
                user_message, retrieval_strategy, context
            )
            ...
            self.logger.info(
                f"🤖 Assistant [{session_id}]: {response_content[:100]}... (took {response_time:.2f}s)"
            )
    ...
    async def _retrieve_information(
        self, query: str, strategy: RetrievalStrategy, context: ConversationContext
    ) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"📊 Retrieved {len(results.get('sources', []))} results in {retrieval_time:.2f}s using {strategy.value}"
            )
    ...
    async def _text2cypher_search(
        ...
        try:
            ...
            enhanced_query = (
                ...
                if conversation_summary != "No previous conversation context."
                else query
            )
                    ...
                    sources.append(
                        {
                            ...
                            "generated_cypher": getattr(
                                self.text2cypher_retriever,
                                "last_generated_cypher",
                                "Not captured",
                            ),
                        }
                    )
            ...
            return {
                ...
                "generated_cypher": getattr(
                    self.text2cypher_retriever, "last_generated_cypher", "Not captured"
                ),
            }

        except Exception as e:
            self.logger.warning(
                f"Text2Cypher search failed, falling back to vector search: {e}"
            )
                ...
                sources.append(
                    {
                        ...
                        "chunk_index": record.get("chunk_index", 0),
                        "score": record.get("score", 0.0),
                        "knowledge_graph_context": record.get(
    ...
    async def _multi_strategy_search(
        ...
        try:
            ...
            if isinstance(rel_result, dict) and "sources" in rel_result:
                all_sources.extend(
                    rel_result["sources"][:3]
                )  # Top 3 from relationships
    ...
    async def _generate_response(
        ...
        try:
            ...
            for i, source in enumerate(
                retrieval_results.get("sources", [])[:5]
            ):  # Top 5 sources
...
class ChatbotCLI:
    ...
    async def run_interactive_session(self):
        ...
        while True:
            try:
                ...
                full_response = ""
                async for chunk in self.chatbot.stream_chat(
    ...
    def _show_history(self):
        ...
        for i, msg in enumerate(history[-10:], 1):  # Show last 10 messages
            ...
            print(
                f"{i}. [{timestamp}] {role_emoji} {msg['role'].title()}: {content_preview}"
            )

            if msg["role"] == "assistant" and msg.get("sources_count", 0) > 0:
                print(
                    f"   └─ Sources: {msg['sources_count']}, Strategy: {msg.get('retrieval_strategy', 'unknown')}"
                )

    ...
    with driver.session() as session:
        ...
        for record in result:
            ...
            entities_by_name[name].append(
                {"id": entity_id, "labels": labels, "type": entity_type}
            )
        ...
        for name, entities in duplicates.items():
            ...
            for i, entity in enumerate(entities, 1):
                print(
                    f"   {i}. ID: {entity['id'][:20]}... | Labels: {entity['labels']} | Type: {entity['type']}"
                )
        ...
        if missing_connections:
            print(
                f"❌ Found {len(missing_connections)} entities that should be connected across chunks:"
            )
            for record in missing_connections:
                print(
                    f"   • '{record['entity_name']}' in Chunk {record['chunk1']} vs Chunk {record['chunk2']}"
                )
        else:
        ...
        print(
            "2. **Fuzzy matching**: Handle variations like 'TechNova' vs 'TechNova Corporation'"
        )
        print("3. **Type-aware merging**: Only merge entities with compatible types")
        print(
            "4. **Cross-chunk linking**: Create SAME_AS relationships between duplicate entities"
        )

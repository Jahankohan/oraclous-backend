...
class EntityResolver:
    ...
    def _find_duplicate_entities(self) -> Dict[str, List[Dict]]:
        ...
        with self.driver.session() as session:
            ...
            duplicates = {name: entities for name, entities in entities_by_name.items() 
                         if len(entities) > 1}
            
            print(f"📊 Found {len(duplicates)} entity names with duplicates")
            return duplicates
    
    def _score_entity_matches(self, duplicates: Dict[str, List[Dict]]) -> List[EntityMatch]:
                ...
                for j in range(i + 1, len(entities)):
                    ...
                    if entity1['original_name'] == entity2['original_name']:
                        ...
                        similarity = self._calculate_name_similarity(
                            entity1['original_name'], 
                            entity2['original_name']
                        )
                        match_type = "fuzzy"
    ...
    def _merge_entities(self, matches: List[EntityMatch]) -> Dict[str, any]:
        ...
        high_confidence_matches = [m for m in matches if m.similarity_score >= self.similarity_threshold]
        ...
        with self.driver.session() as session:
            for match in high_confidence_matches:
                try:
                    ...
                    """, entity1_id=match.entity1_id, entity2_id=match.entity2_id)
                    
                    merged_count += 1
                    print(f"   ✅ Merged '{match.entity2_name}' (Chunk {match.entity2_chunk}) → '{match.entity1_name}' (Chunk {match.entity1_chunk})")
                    
                except Exception as e:
                    print(f"   ❌ Failed to merge {match.entity1_name} ↔ {match.entity2_name}: {e}")
    ...
    def _create_cross_chunk_links(self, matches: List[EntityMatch]) -> Dict[str, any]:
        ...
        medium_confidence_matches = [m for m in matches 
                                   if 0.7 <= m.similarity_score < self.similarity_threshold]
        
        print(f"🔗 Creating cross-chunk links for {len(medium_confidence_matches)} medium-confidence matches")
        ...
        with self.driver.session() as session:
            for match in medium_confidence_matches:
                try:
                    ...
                    """, entity1_id=match.entity1_id, entity2_id=match.entity2_id,
                         similarity=match.similarity_score, match_type=match.match_type)
                    
                    links_created += 1
                    print(f"   🔗 Linked '{match.entity1_name}' ↔ '{match.entity2_name}' (similarity: {match.similarity_score:.2f})")
                    
                except Exception as e:
                    print(f"   ❌ Failed to link {match.entity1_name} ↔ {match.entity2_name}: {e}")

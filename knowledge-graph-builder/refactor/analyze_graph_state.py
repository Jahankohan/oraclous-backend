...
def main():
    ...
    try:
        ...
        print(f"FROM_CHUNK relationships found: {len(connections['from_chunk_connections'])}")
        if connections['from_chunk_connections']:
            print("\nSample FROM_CHUNK connections:")
            for conn in connections['from_chunk_connections'][:5]:
                print(f"  {conn['entity_labels'][0] if conn['entity_labels'] else 'Unknown'}: {conn['entity_name']} -> Chunk {conn['chunk_index']}")
        ...
        if connections['from_document_connections']:
            ...
            for conn in connections['from_document_connections'][:5]:
                print(f"  Chunk {conn['chunk_index']} -> Document {conn['document_id']}")
        ...
        if connections['complete_traceability_chains']:
            ...
            for chain in connections['complete_traceability_chains'][:5]:
                entity_label = chain['entity_labels'][0] if chain['entity_labels'] else 'Unknown'
                print(f"  {entity_label}: {chain['entity_name']} -> Chunk {chain['chunk_index']} -> Doc {chain['document_id']}")
        ...
        if entities:
            for entity in entities[:10]:
                labels_str = ":".join(entity['labels']) if entity['labels'] else "Unknown"
        ...
        if schema['relationships']:
            print("Relationship Types:", [rel.get('type') for rel in schema['relationships']])
        
        # 5. Final assessment
        print("\n✅ Assessment:")
        if stats['total_nodes'] == 0:
            print("❌ No data found in database - pipeline hasn't processed any documents yet")
        elif len(connections['from_chunk_connections']) == 0:
            print("⚠️  Entities found but no FROM_CHUNK relationships - connection issue detected")
        elif len(connections['complete_traceability_chains']) > 0:
            print("✅ Complete traceability chains found - entity-chunk-document connections working!")
        else:

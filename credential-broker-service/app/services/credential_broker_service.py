...
class CredentialBroker:
    ...
    async def get_provider_token(
        ...
        try:
            ...
            token_result = await self._get_runtime_token(
                user_id, provider, state, required_scopes
            )
            if not token_result.success:
                # If token expired, try to refresh
                if token_result.error_code == OAUTH_ERROR_CODES["TOKEN_EXPIRED"]:
                    refresh_result = await self.refresh_token_if_needed(
                        user_id, provider
                    )
            ...
            if required_scopes:
                ...
                if missing:
                    ...
                    scope_result = await self._validate_scopes(
                        user_id, provider, required_scopes
                    )
                    if not scope_result.success:
                        return scope_result
            print(
                f"Successfully obtained token for user {user_id}, provider {provider}"
            )
            return token_result
    ...
    async def ensure_data_source_access(
        self, user_id: UUID, provider: str, required_scopes: str
    ) -> CredentialResult:
        """
        ...
        try:
            ...
            capabilities = DATA_SOURCE_CAPABILITIES.get(provider, {}).get(
                required_scopes
            )
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"Failed to list user providers: {response.status_code}"
                    )
                    return {}
    ...
    async def _get_login_url(
        self, provider: str, state: str, required_scopes
    ) -> Optional[str]:
        async with httpx.AsyncClient() as client:
            response = await client.get(
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    login_url = await self._get_login_url(
                        provider, state, required_scopes
                    )
                    return CredentialResult(
                        success=False,

...
from app.schema.credential_schema import (
    CreateCredential,
    RequestCredentials,
    RequestCredentialsResponse,
    CredentialOut,
    CredentialsUpdate,
    CredentialBrokerSuccessResponse,
    CredentialBrokerErrorResponse,
    ProvidersResponse,
...
async def get_credential(
    cred_id: UUID, repo: CredentialRepository = Depends(get_credential_repo)
):
    credential_service = CredentialService(repo)
...
async def list_credentials(
    credential_request: RequestCredentials,
    repo: CredentialRepository = Depends(get_credential_repo),
):
    credential_service = CredentialService(repo)
...
@router.post(
    "/runtime-token",
    response_model=CredentialBrokerSuccessResponse | CredentialBrokerErrorResponse,
)
async def get_runtime_token(request: Request, request_data: RuntimeTokenRequest):
    broker = CredentialBroker()
    result = await broker.get_provider_token(
        request_data.user_id, request_data.provider, request_data.required_scopes
    )
    if result.success:
...
async def list_user_providers(request: Request, user_id: UUID = Query(...)):
    broker = CredentialBroker()
...
async def get_available_data_sources(request: Request, user_id: UUID = Query(...)):
    broker = CredentialBroker()
...
@router.post(
    "/ensure-data-source-access",
    response_model=CredentialBrokerSuccessResponse | CredentialBrokerErrorResponse,
)
async def ensure_data_source_access(
    request: Request, request_data: RuntimeTokenRequest
):
    broker = CredentialBroker()
    result = await broker.ensure_data_source_access(
        request_data.user_id, request_data.provider, request_data.required_scopes
    )
    if result.success:
        return CredentialBrokerSuccessResponse(

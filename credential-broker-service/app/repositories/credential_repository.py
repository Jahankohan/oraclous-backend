...
class CredentialRepository:
    ...
    async def create_credential(
        self, user_credential: CreateCredential
    ) -> UserCredential:
        cred = UserCredential(
    ...
    async def list_credentials(
        self, cred_request: RequestCredentials
    ) -> List[UserCredential]:
        async with self.Session() as session:
            stmt = select(UserCredential).where(
                UserCredential.user_id == cred_request.user_id
            )
    ...
    async def get_credential_by_id(self, cred_id: UUID) -> Optional[UserCredential]:
        async with self.Session() as session:
            result = await session.execute(
                select(UserCredential).where(UserCredential.id == cred_id)
            )
    ...
    async def update_credential(self, u: CredentialsUpdate) -> UserCredential:
        async with self.Session.begin() as session:
            ...
            if u.name is not None:
                obj.name = u.name
            if u.provider is not None:
                obj.provider = u.provider
            if u.user_id is not None:
                obj.user_id = u.user_id
            if u.tool_id is not None:
                obj.tool_id = u.tool_id
            if u.cred_type is not None:
                obj.cred_type = CredentialType(u.cred_type)
            if u.credential is not None:
                obj.encrypted_cred = u.credential  # already encrypted

...
class OAuthService:
    ...
    async def build_login_url(
        self, provider: str, state: str, required_scopes: Optional[List[str]] = None
    ) -> str:
    ...
    async def exchange_token(self, provider: str, code: str):
        ...
        if provider == "notion":
            ...
            auth = (
                ...
                os.getenv(f"{provider.upper()}_CLIENT_SECRET"),
            )
        ...
        async with httpx.AsyncClient() as client:
            resp = await client.post(
                config["token_url"], data=data, headers=headers, auth=auth
            )
    ...
    async def ensure_access(
        ...
        if not token_obj:
            ...
            login_url = await self.build_login_url(
                provider, redirect_state, required_scopes
            )
            return {
        ...
        if missing_scopes:
            ...
            login_url = await self.build_login_url(
                provider, redirect_state, combined_scopes
            )
            ensure_access_response = EnsureAccessResponse(
                action="reauthenticate",
                login_url=login_url,
    ...
    async def validate_token_scopes(
        ...
        missing_scopes = [s for s in required_scopes if s not in current_scopes]
        token_expired = (
            token_obj.expires_at and datetime.utcnow() > token_obj.expires_at
        )

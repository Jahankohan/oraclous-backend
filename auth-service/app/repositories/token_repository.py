...
class TokenRepository:
    ...
    async def get_token(self, user_id: str, provider: str) -> Optional[OauthTokens]:
        ...
        try:
            async with self.Session() as session:
                result = await session.execute(
                    select(OauthTokens).where(
                        OauthTokens.user_id == user_id, OauthTokens.provider == provider
                    )
                )
                ...
                if token:
                    logger.debug(
                        f"Retrieved token for user {user_id}, provider {provider}"
                    )
                else:
                    logger.info(
            ...
            logger.error(
                f"Database error retrieving token for {user_id}/{provider}: {e}"
            )
            raise
        except Exception as e:
            logger.error(
                f"Unexpected error retrieving token for {user_id}/{provider}: {e}"
            )
            raise
    ...
    async def save_token(
        ...
        try:
            async with self.Session() as session:
                ...
                logger.info(
                    f"Saved token for user {user_id}, provider {provider} with scopes: {list(merged_scopes)}"
                )
    ...
    async def update_access_token(
        self, user_id: str, provider: str, access_token: str, expires_at: datetime
    ):
        ...
        try:
            async with self.Session() as session:
                ...
                if token:
                    ...
                    await session.commit()
                    logger.info(
                        f"Updated access token for user {user_id}, provider {provider}"
                    )
                else:
                    logger.warning(
                        f"No token found to update for user {user_id}, provider {provider}"
                    )

        except SQLAlchemyError as e:
            logger.error(
                f"Database error updating access token for {user_id}/{provider}: {e}"
            )
            raise
        except Exception as e:
            logger.error(
                f"Unexpected error updating access token for {user_id}/{provider}: {e}"
            )
            raise
        ...
        try:
            async with self.Session() as session:
                ...
                if token:
                    ...
                    await session.commit()
                    logger.info(
                        f"Deleted token for user {user_id}, provider {provider}"
                    )
                    return True
                else:
                    logger.warning(
                        f"No token found to delete for user {user_id}, provider {provider}"
                    )
                    return False
            ...
            logger.error(
                f"Unexpected error deleting token for {user_id}/{provider}: {e}"
            )
            raise
    ...
    async def get_tokens_expiring_soon(self, minutes: int = 30) -> List[OauthTokens]:
        ...
        try:
            ...
            async with self.Session() as session:
                ...
                tokens = result.scalars().all()
                logger.debug(
                    f"Found {len(tokens)} tokens expiring within {minutes} minutes"
                )
                return tokens
    ...
    async def validate_token_scopes(
        self, user_id: str, provider: str, required_scopes: List[str]
    ) -> dict:
        ...
        try:
            ...
            missing_scopes = [
                scope for scope in required_scopes if scope not in current_scopes
            ]

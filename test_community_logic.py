...
def test_community_summary_generation():
    ...
    tech_members = [
        {"entity_id": "1", "entity_name": "Machine Learning", "entity_labels": ["__Entity__", "Technology"]},
        {"entity_id": "2", "entity_name": "Neural Networks", "entity_labels": ["__Entity__", "Technology"]},
        {"entity_id": "3", "entity_name": "Deep Learning", "entity_labels": ["__Entity__", "Technology"]}
    ]
    ...
    mixed_members = [
        {"entity_id": "4", "entity_name": "Python", "entity_labels": ["__Entity__", "Programming"]},
        {"entity_id": "5", "entity_name": "Data Science", "entity_labels": ["__Entity__", "Field"]},
        {"entity_id": "6", "entity_name": "Statistics", "entity_labels": ["__Entity__", "Mathematics"]},
        {"entity_id": "7", "entity_name": "Analytics", "entity_labels": ["__Entity__", "Field"]},
        {"entity_id": "8", "entity_name": "Research", "entity_labels": ["__Entity__", "Field"]}
    ]
    ...
    many_members = [
        {"entity_id": f"entity_{i}", "entity_name": f"Entity {i}", "entity_labels": ["__Entity__"]}
        for i in range(10)
    ]
    ...
    if ("technology" in summary_1.lower() and 
        "machine learning" in summary_1.lower() and
        "3 entities" in summary_1.lower()):
        print("✅ Technology community summary is correct")
    else:
        print("❌ Technology community summary is incorrect")
        return False
    
    if ("field" in summary_2.lower() and 
        "5 entities" in summary_2.lower() and
        "and 2 others" in summary_2.lower()):
        print("✅ Mixed community summary is correct")
    else:
        print("❌ Mixed community summary is incorrect")
        return False
    
    if ("10" in summary_3 and "entities" in summary_3 and
        "and 7 others" in summary_3):
...
def test_community_data_structure():
    ...
    required_fields = ["communities_created", "relationships_created", "graph_id", "algorithm_used"]
    ...
    print(f"   Example search result: {len(search_result['communities'])} communities found")
    
    # Verify community structure
    if search_result["communities"]:
        community = search_result["communities"][0]
        expected_fields = ["community_id", "summary", "entity_count", "weight", "members"]
